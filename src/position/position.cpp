...
namespace stormphrax {
        ...
        auto scharnaglToBackrank(u32 n) {
            ...
            const auto placeInNthFree = [&dst](u32 n, PieceType piece) {
                for (i32 free = 0, i = 0; i < 8; ++i) {
                    if (dst[i] == PieceType::Pawn && free++ == n) {
                        dst[i] = piece;
                        break;
                    }
                }
    ...
    template auto
    Position::movePiece<false, false>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::movePiece<true, false>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::movePiece<false, true>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::movePiece<true, true>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;

    template auto
    Position::promotePawn<false, false>(Piece, Square, Square, PieceType, eval::NnueUpdates &)
        -> Piece;
    template auto
    Position::promotePawn<true, false>(Piece, Square, Square, PieceType, eval::NnueUpdates &)
        -> Piece;
    template auto
    Position::promotePawn<false, true>(Piece, Square, Square, PieceType, eval::NnueUpdates &)
        -> Piece;
    template auto
    Position::promotePawn<true, true>(Piece, Square, Square, PieceType, eval::NnueUpdates &)
        -> Piece;

    template auto
    Position::castle<false, false>(Piece, Square, Square, eval::NnueUpdates &) -> void;
    ...
    template auto
    Position::enPassant<false, false>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::enPassant<true, false>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::enPassant<false, true>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    template auto
    Position::enPassant<true, true>(Piece, Square, Square, eval::NnueUpdates &) -> Piece;
    ...
    auto Position::resetFromFen(const std::string &fen) -> bool {
        ...
        for (const auto &rank: ranks) {
            ...
            for (const auto c: rank) {
                if (fileIdx >= 8) {
                    std::cerr << "too many files in rank " << rankIdx << " in fen " << fen
                              << std::endl;
                    return false;
                }
            ...
            if (fileIdx < 8) {
                std::cerr << "not enough files in rank " << rankIdx << " in fen " << fen
                          << std::endl;
                return false;
            }
        ...
        if (const auto blackKingCount = newBbs.forPiece(Piece::BlackKing).popcount();
            blackKingCount != 1)
        {
            std::cerr << "black must have exactly 1 king, " << blackKingCount << " in fen " << fen
                      << std::endl;
            return false;
        }
        ...
        if (const auto whiteKingCount = newBbs.forPiece(Piece::WhiteKing).popcount();
            whiteKingCount != 1)
        {
            std::cerr << "white must have exactly 1 king, " << whiteKingCount << " in fen " << fen
                      << std::endl;
            return false;
        }
        ...
        if (const auto stm = newBlackToMove ? Color::Black : Color::White; isAttacked<false>(
                newState,
                stm,
            ...
            if (g_opts.chess960) {
                ...
                for (const auto flag: castlingFlags) {
                    if (flag >= 'a' && flag <= 'h') {
                        ...
                        if (file == kingFile) {
                            std::cerr << "invalid castling availability in fen " << fen
                                      << std::endl;
                            return false;
                        }
                        ...
                        if (file < kingFile)
                            ...
                            newState.castlingRooks.black().kingside = toSquare(7, file);
                        ...
                        if (file < kingFile)
                            ...
                            newState.castlingRooks.white().kingside = toSquare(0, file);
                    } else if (flag == 'k') {
                ...
                for (const auto flag: castlingFlags) {
                    switch (flag) {
                        case 'k':
                            newState.castlingRooks.black().kingside = Square::H8;
                            break;
                        case 'q':
                            newState.castlingRooks.black().queenside = Square::A8;
                            break;
                        case 'K':
                            newState.castlingRooks.white().kingside = Square::H1;
                            break;
                        case 'Q':
                            newState.castlingRooks.white().queenside = Square::A1;
                            break;
                        default:
                            std::cerr << "invalid castling availability in fen " << fen
                                      << std::endl;
        ...
        if (enPassant != "-") {
            if (newState.enPassant = squareFromString(enPassant);
                newState.enPassant == Square::None)
            {
    ...
    auto Position::applyMoveUnchecked(Move move, eval::NnueState *nnueState) -> void {
        ...
        switch (moveType) {
            ...
            case MoveType::Promotion:
                captured =
                    promotePawn<true, UpdateNnue>(moving, moveSrc, moveDst, move.promo(), updates);
                break;
        ...
        if (captured == Piece::None && pieceType(moving) != PieceType::Pawn)
            ++state.halfmove;
        else
            state.halfmove = 0;
    ...
    auto Position::isPseudolegal(Move move) const -> bool {
        ...
        if (type == MoveType::Castling) {
            ...
            if (g_opts.chess960) {
                ...
                return (castleOcc & (toKingDst | toRook | squareBit(kingDst) | squareBit(rookDst)))
                           .empty()
                ...
                if (dst == state.castlingRooks.black().kingside)
                    ...
                    return (occ & U64(0x000000000000000E)).empty()
        ...
        if (srcPieceType == PieceType::Pawn) {
            if (type == MoveType::EnPassant)
                return dst == state.enPassant
                    && attacks::getPawnAttacks(state.enPassant, them)[src];
            ...
            if (delta == 2
                && occ[static_cast<Square>(
                    static_cast<i32>(dst) + (us == Color::White ? offsets::Down : offsets::Up)
                )])
                return false;
        } else {
    ...
    auto Position::isLegal(Move move) const -> bool {
        ...
        if (move.type() == MoveType::Castling) {
            ...
            const auto postEpOcc = bbs.occupancy() ^ Bitboard::fromSquare(src)
                                 ^ Bitboard::fromSquare(dst) ^ Bitboard::fromSquare(captureSquare);

            const auto theirQueens = bbs.queens(them);

            return (attacks::getBishopAttacks(king, postEpOcc) & (theirQueens | bbs.bishops(them)))
                       .empty()
                && (attacks::getRookAttacks(king, postEpOcc) & (theirQueens | bbs.rooks(them)))
                       .empty();
        }
        ...
        if (pieceType(moving) == PieceType::King) {
            ...
            return !state.threats[move.dst()]
                && (attacks::getBishopAttacks(dst, kinglessOcc) & (theirQueens | bbs.bishops(them)))
                       .empty()
                && (attacks::getRookAttacks(dst, kinglessOcc) & (theirQueens | bbs.rooks(them)))
                       .empty();
        }

        // multiple checks can only be evaded with a king move
        if (state.checkers.multiple() || state.pinned[src] && !rayIntersecting(src, dst)[king])
            return false;
    ...
    auto Position::hasCycle(i32 ply) const -> bool {
        ...
        const auto S = [this](i32 d) { return m_keys[m_keys.size() - d]; };
        ...
        for (i32 rank = 7; rank >= 0; --rank) {
            for (i32 file = 0; file < 8; ++file) {
                if (state.boards.pieceAt(rank, file) == Piece::None) {
                    u32 emptySquares = 1;
                    for (; file < 7 && state.boards.pieceAt(rank, file + 1) == Piece::None;
                         ++file, ++emptySquares)
                    {}

                    fen << static_cast<char>('0' + emptySquares);
                } else
                    fen << pieceToChar(state.boards.pieceAt(rank, file));
            }
    ...
    auto Position::movePiece(Piece piece, Square src, Square dst, eval::NnueUpdates &nnueUpdates)
        -> Piece {
        assert(piece != Piece::None);
    ...
    template<bool UpdateKey, bool UpdateNnue>
    auto Position::promotePawn(
        Piece pawn,
        Square src,
        Square dst,
        PieceType promo,
        eval::NnueUpdates &nnueUpdates
    ) -> Piece {
    ...
    template<bool UpdateKey, bool UpdateNnue>
    auto
    Position::castle(Piece king, Square kingSrc, Square rookSrc, eval::NnueUpdates &nnueUpdates)
        -> void {
    ...
    template<bool UpdateKey, bool UpdateNnue>
    auto Position::enPassant(Piece pawn, Square src, Square dst, eval::NnueUpdates &nnueUpdates)
        -> Piece {
        assert(pawn != Piece::None);
        assert(pieceType(pawn) == PieceType::Pawn);
