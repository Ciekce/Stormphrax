...
namespace stormphrax {
    ...
    Position Position::applyMove(Move move, eval::NnueState* nnueState) const {
        ...
        if (captured == Piece::kNone && pieceType(moving) != PieceTypes::kPawn) {
            ++newPos.m_halfmove;
        } else {
            newPos.m_halfmove = 0;
        }

        if (captured != Piece::kNone && pieceType(captured) == PieceTypes::kRook) {
    ...
    bool Position::isPseudolegal(Move move) const {
        ...
        if (dstPiece != Piece::kNone
            ...
            && ((pieceColor(dstPiece) == us
                ...
                || pieceType(dstPiece) == PieceTypes::kKing))
        {
            return false;
        }
        ...
        if (type == MoveType::kCastling) {
            ...
            if (g_opts.chess960) {
                ...
                if (dst == m_castlingRooks.black().kingside) {
                    return (occ & U64(0x6000000000000000)).empty() && !isAttacked(Square::kF8, Colors::kWhite);
                } else if (dst == m_castlingRooks.black().queenside) {
                    return (occ & U64(0x0E00000000000000)).empty() && !isAttacked(Square::kD8, Colors::kWhite);
                } else if (dst == m_castlingRooks.white().kingside) {
                    return (occ & U64(0x0000000000000060)).empty() && !isAttacked(Square::kF1, Colors::kBlack);
                } else {
                    return (occ & U64(0x000000000000000E)).empty() && !isAttacked(Square::kD1, Colors::kBlack);
                }
            }
        }
        ...
        if (srcPieceType == PieceTypes::kPawn) {
            ...
            if (delta == 2
                && occ[static_cast<Square>(
                    static_cast<i32>(dst) + (us == Colors::kWhite ? offsets::kDown : offsets::kUp)
                )])
            {
                return false;
            }
    ...
    bool Position::isLegal(Move move) const {
        ...
        if (pieceType(moving) == PieceTypes::kKing) {
    ...
    void Position::setPiece(Piece piece, Square square) {
        ...
        assert(pieceType(piece) != PieceTypes::kKing);
    ...
    void Position::movePieceNoCap(Piece piece, Square src, Square dst) {
        ...
        if (pieceType(piece) == PieceTypes::kKing) {
    ...
    Piece Position::movePiece(Piece piece, Square src, Square dst, eval::NnueUpdates& nnueUpdates) {
        ...
        if (captured != Piece::kNone) {
            assert(pieceType(captured) != PieceTypes::kKing);
    ...
    Piece Position::promotePawn(Piece pawn, Square src, Square dst, PieceType promo, eval::NnueUpdates& nnueUpdates) {
        ...
        assert(pieceType(pawn) == PieceTypes::kPawn);
    ...
    void Position::castle(Piece king, Square kingSrc, Square rookSrc, eval::NnueUpdates& nnueUpdates) {
        ...
        assert(pieceType(king) == PieceTypes::kKing);
        ...
        const auto rook = copyPieceColor(king, PieceTypes::kRook);
    ...
    void Position::filterEp(Color capturing) {
        ...
        const auto movedPawn =
            toSquare(squareRank(m_enPassant) + (moved == Colors::kWhite ? 1 : -1), squareFile(m_enPassant));
    ...
    Move Position::moveFromUci(std::string_view move) const {
        ...
        if (move.length() == 5) {
            ...
            if (srcPiece == Piece::kBlackKing || srcPiece == Piece::kWhiteKing) {
                if (g_opts.chess960) {
                    if (m_boards.pieceOn(dst) == copyPieceColor(srcPiece, PieceTypes::kRook)) {
            ...
            if (g_opts.chess960) {
                for (i32 rank = 0; rank < 8; ++rank) {
                    for (i32 file = 0; file < 8; ++file) {
                        ...
                        if (piece != Piece::kNone && pieceType(piece) == PieceTypes::kKing) {
        ...
        if (pos.m_enPassant != Square::kNone) {
            [&] {
                ...
                const auto pawnSquare = toSquare(
                    squareRank(pos.m_enPassant) + (pos.m_stm == Colors::kBlack ? 1 : -1),
