...
namespace stormphrax::search {
    ...
    auto Searcher::startSearch(
        ...
        if (m_ttable.finalize()) {
            ...
            std::cout << "info string No ucinewgame or isready before go, lost "
                      << static_cast<u32>(initTime * 1000.0) << " ms to TT initialization"
                      << std::endl;
        }
    ...
    auto Searcher::searchRoot(ThreadData &thread, bool actualSearch) -> Score {
        ...
        for (i32 depth = 1;; ++depth) {
            ...
            while (!hasStopped()) {
                ...
                newScore =
                    search<true, true>(thread, thread.rootPv, aspDepth, 0, 0, alpha, beta, false);
    ...
    auto Searcher::search(
        ...
        if (depth >= minIirDepth() && !curr.excluded && (PvNode || cutnode)
        ...
        if (!curr.excluded) {
            ...
            if (!ttHit) {
                m_ttable
                    .put(pos.key(), ScoreNone, rawStaticEval, NullMove, 0, 0, TtFlag::None, ttpv);
            }
        ...
        if (!PvNode && !inCheck && !curr.excluded) {
                ...
                && !bbs.nonPk(us).empty())
            {
                ...
                const auto R = nmpBaseReduction() + depth / nmpDepthReductionDiv()
                             + std::min(
                                   (curr.staticEval - beta) / nmpEvalReductionScale(),
                                   maxNmpEvalReduction()
                             )
                             + improving;
                ...
                && !(ttHit && ttEntry.depth >= probcutDepth && ttEntry.score < probcutBeta))
            {
                ...
                auto generator = MoveGenerator::probcut(
                    pos,
                    ttEntry.move,
                ...
                while (const auto move = generator.next()) {
                    ...
                    auto score =
                        -qsearch(thread, ply + 1, moveStackIdx + 1, -probcutBeta, -probcutBeta + 1);
        ...
        const auto lmrMinMoves = RootNode ? lmrMinMovesRoot()
        ...
        while (const auto move = generator.next()) {
            ...
            if (RootNode && g_opts.showCurrMove && elapsed() > CurrmoveReportDelay) {
                std::cout << "info depth " << depth << " currmove " << uci::moveToString(move)
                ...
                && ttEntry.flag != TtFlag::UpperBound)
            {
                ...
                if (score < sBeta) {
                    if (!PvNode && curr.multiExtensions <= multiExtLimit()
            ...
            if (pos.isDrawn(true)) {
                ...
                if (depth >= minLmrDepth() && legalMoves >= lmrMinMoves && quietOrLosing) {
                    auto r = baseLmr;
                    ...
                    if (score > alpha && reduced < newDepth) {
                        ...
                        if (!noisy && (score <= alpha || score >= beta)) {
                            const auto bonus =
                                score <= alpha ? historyPenalty(newDepth) : historyBonus(newDepth);
                            thread.history
                                .updateConthist(thread.conthist, ply, moving, move, bonus);
                        }
                    }
                }
        ...
        if (!curr.excluded && !hasStopped()) {
            ...
            {
                thread.correctionHistory
                    .update(pos, thread.contMoves, ply, depth, bestScore, curr.staticEval);
            }
    ...
    auto Searcher::qsearch(ThreadData &thread, i32 ply, u32 moveStackIdx, Score alpha, Score beta)
        -> Score {
        assert(ply > 0 && ply <= MaxDepth);
        ...
        if (pos.isCheck()) {
            ...
            const auto staticEval = eval::adjustEval(
                pos,
                thread.contMoves,
                ply,
                &thread.correctionHistory,
    ...
    auto Searcher::report(
        ...
        std::cout << "info depth " << depth << " seldepth " << seldepth << " time " << ms
        ...
        if (std::abs(score) >= ScoreMaxMate) {
            if (score > 0) {
                ...
                std::cout << "mate " << (-(ScoreMate + score) / 2);
            }
        } else {
