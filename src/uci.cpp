...
namespace stormphrax {
    ...
    namespace {
        ...
        auto UciHandler::handleUci() -> void {
            ...
            std::cout << "option name Hash type spin default " << DefaultTtSize << " min "
                      << TtSizeRange.min() << " max " << TtSizeRange.max() << '\n';
            std::cout << "option name Clear Hash type button\n";
            std::cout << "option name Threads type spin default " << opts::DefaultThreadCount
                      << " min " << opts::ThreadCountRange.min() << " max "
                      << opts::ThreadCountRange.max() << '\n';
            std::cout << "option name Contempt type spin default "
                      << opts::DefaultNormalizedContempt << " min " << ContemptRange.min()
                      << " max " << ContemptRange.max() << '\n';
            ...
            std::cout << "option name Move Overhead type spin default "
                      << limit::DefaultMoveOverhead << " min " << limit::MoveOverheadRange.min()
                      << " max " << limit::MoveOverheadRange.max() << '\n';
            std::cout << "option name SoftNodes type check default " << defaultOpts.softNodes
                      << std::endl;
            ...
            std::cout << "option name EnableWeirdTCs type check default "
                      << defaultOpts.enableWeirdTcs << std::endl;
            std::cout << "option name SyzygyPath type string default <empty>\n";
            std::cout << "option name SyzygyProbeDepth type spin default "
                      << defaultOpts.syzygyProbeDepth << " min "
                      << search::SyzygyProbeDepthRange.min() << " max "
                      << search::SyzygyProbeDepthRange.max() << '\n';
            std::cout << "option name SyzygyProbeLimit type spin default "
                      << defaultOpts.syzygyProbeLimit << " min "
                      << search::SyzygyProbeLimitRange.min() << " max "
                      << search::SyzygyProbeLimitRange.max() << '\n';
            ...
            for (const auto& param: tunableParams()) {
                std::cout << "option name " << param.name << " type spin default "
                          << param.defaultValue << " min " << param.range.min() << " max "
                    ...
                    if (tokens[i] == "nodes" && ++i < tokens.size()) {
                        ...
                        if (!util::tryParseI64(time, tokens[i])) {
                            ...
                            limiter->addLimiter<limit::MoveTimeLimiter>(time, m_moveOverhead);
                               ...
                               && tokens[i - 1]
                                      == (m_pos.toMove() == Color::Black ? "btime" : "wtime"))
                    {
                        tournamentTime = true;
                        ...
                        if (!util::tryParseI64(time, tokens[i])) {
                            ...
                            timeRemaining = static_cast<i64>(time);
                               ...
                               && tokens[i - 1]
                                      == (m_pos.toMove() == Color::Black ? "binc" : "winc"))
                    {
                        tournamentTime = true;
                        ...
                        while (i + 1 < tokens.size()) {
                            ...
                            if (candidate.length() >= 4 && candidate.length() <= 5
                                && candidate[0] >= 'a' && candidate[0] <= 'h' && candidate[1] >= '1'
                                && candidate[1] <= '8' && candidate[2] >= 'a' && candidate[2] <= 'h'
                                && candidate[3] >= '1' && candidate[3] <= '8'
                                && (candidate.length() < 5
                                    || isValidPromotion(pieceTypeFromChar(candidate[4]))))
                            {
                                ...
                                if (std::ranges::find(movesToSearch, move) == movesToSearch.end()) {
                                    if (m_pos.isPseudolegal(move) && m_pos.isLegal(move)) {
                                        ...
                                        std::cout << "info string ignoring illegal move "
                                                  << candidate << std::endl;
                                    }
                                }
        ...
        auto UciHandler::handleSetoption(const std::vector<std::string>& tokens) -> void {
            ...
            if (!nameEmpty) {
                std::transform(nameStr.begin(), nameStr.end(), nameStr.begin(), [](auto c) {
                    ...
                    if (!valueEmpty) {
                        if (const auto newSoftNodeHardLimitMultiplier = util::tryParseI32(valueStr))
                        {
                            opts::mutableOpts().softNodeHardLimitMultiplier =
                                limit::SoftNodeHardLimitMultiplierRange.clamp(
                                    *newSoftNodeHardLimitMultiplier
                                );
                        }
                    }
        ...
        auto UciHandler::handleD() -> void {
            ...
            const auto staticEval =
                eval::adjustEval<false>(m_pos, {}, 0, nullptr, eval::staticEvalOnce(m_pos));
        ...
        auto UciHandler::handleBench(const std::vector<std::string>& tokens) -> void {
            ...
            if (tokens.size() > 2) {
                if (const auto newThreads = util::tryParseU32(tokens[2])) {
                    if (*newThreads > 1) {
                        std::cout
                            << "info string multiple search threads not yet supported, using 1"
                            << std::endl;
                    }
                } else {
    ...
    namespace uci {
        ...
        auto moveToString(Move move) -> std::string {
            ...
            if (type != MoveType::Castling || g_opts.chess960) {
                ...
                const auto dst = move.srcFile() < move.dstFile() ? toSquare(move.srcRank(), 6)
        ...
        auto printCttTuningParams(std::span<const std::string> params) -> void {
            ...
            const auto printParam = [&first](const auto& param) {
                ...
                std::cout << "\"" << param.name << "\": \"Integer(" << param.range.min() << ", "
                          << param.range.max() << ")\"";
        ...
        auto printObTuningParams(std::span<const std::string> params) -> void {
            const auto printParam = [](const auto& param) {
                std::cout << param.name << ", int, " << param.value << ".0, " << param.range.min()
                          << ".0, " << param.range.max() << ".0, " << param.step << ", 0.002"
                          << std::endl;
            };
