...
namespace stormphrax {
    ...
    public:
        ...
        inline auto update(
            ...
            m_blackNonPawnTable[stm][pos.blackNonPawnKey() % Entries].update(
                scaledError,
                newWeight
            );
            m_whiteNonPawnTable[stm][pos.whiteNonPawnKey() % Entries].update(
                scaledError,
                newWeight
            );
        ...
        correct(const Position &pos, std::span<search::PlayedMove> moves, i32 ply, Score score)
            ...
            if (ply >= 2) {
                ...
                if (moving2 != Piece::None && moving1 != Piece::None) {
                    correction +=
                        contCorrhistWeight()
                        * m_contTable[stm][static_cast<i32>(pieceType(moving2))]
                                     [static_cast<i32>(dst2)][static_cast<i32>(pieceType(moving1))]
