...
namespace stormphrax::numa {
    ...
    template <typename T>
    void NumaUniqueAllocation<T>::realloc(usize count) {
        free();
        alloc(count);
    }

    template <typename T>
    void NumaUniqueAllocation<T>::alloc(usize count) {
    ...
    template <typename T>
    void NumaUniqueAllocation<T>::free() {
        for (auto* ptr : m_data) {
    ...
    void NumaUniqueAllocation<T>::alloc(usize count) {
        m_data.reserve(1);
        auto* ptr = util::alignedAlloc<T>(std::max<usize>(4096, alignof(T)), count);
        for (usize i = 0; i < count; ++i) {
            new (&ptr[i]) T;
        }
        m_data.push_back(ptr);
