	...
	private:
			...
			const BitboardSet &bbs, RefreshTable &refreshTable, Square king) -> void
		{
			...
			while (adds.size() >= 4) {
				...
				rtEntry.accumulator.activateFourFeatures(g_network.featureTransformer(), c, add0,
				 																			add1,
				 																			add2,
				 																			add3);
			}
			...
			while (subs.size() >= 4) {
				...
				rtEntry.accumulator.deactivateFourFeatures(g_network.featureTransformer(), c, sub0,
				 																			  sub1,
				 																			  sub2,
				 																			  sub3);
			}
