...
namespace stormphrax::eval::psqt {
#define S(Mg, Eg) \
    TaperedScore { \
        (Mg), (Eg) \
    }
    namespace {
        // clang-format off
        constexpr auto kBonusTables = std::array{
            std::array{ // pawns
        ...
        std::array<std::array<TaperedScore, Squares::kCount>, Pieces::kCount> createPsts() {
            std::array<std::array<TaperedScore, Squares::kCount>, Pieces::kCount> psts{};

            for (const auto piece : PieceTypes::kAll) {
                const auto value = pieceValue(piece);
                const auto& bonusTable = kBonusTables[piece.idx()];

                auto& blackPst = psts[piece.idx() * 2];
                auto& whitePst = psts[piece.idx() * 2 + 1];
...
#undef S
    ...
    const std::array<std::array<TaperedScore, Squares::kCount>, Pieces::kCount> g_pieceSquareTables = createPsts();
} // namespace stormphrax::eval::psqt
