...
namespace stormphrax::eval::nnue::layers {
    ...
    struct BaseAffine {
        ...
        static_assert(
            sizeof(InputType) * InputCount >= util::simd::Alignment
            && (sizeof(InputType) * InputCount) % util::simd::Alignment == 0
        );

        static_assert(
            sizeof(ParamType) * WeightCount >= util::simd::Alignment
    ...
    struct DenseAffine :
            BaseAffine<
                Input,
                Param,
                typename Activation::OutputType,
                Inputs,
                Inputs,
                Outputs,
                OutputBucketing> {
        ...
        inline auto forward(
            ...
            for (u32 outputIdx = 0; outputIdx < Base::OutputCount; ++outputIdx) {
                ...
                for (u32 inputIdx = 0; inputIdx < Base::InputCount; inputIdx += ChunkSize) {
                    const auto inputVec =
                        util::simd::load<typename Base::InputType>(&inputs[inputIdx]);
                ...
                const auto bias = static_cast<typename Base::OutputType>(
                    Base::biases[bucketBiasOffset + outputIdx]
                );
            ...
            for (u32 outputIdx = 0; outputIdx < Base::OutputCount; ++outputIdx) {
                ...
                for (u32 inputIdx = 0; inputIdx < PerspectiveInputCount; inputIdx += ChunkSize) {
                    ...
                    const auto weightVec =
                        load<typename Base::ParamType>(&Base::weights[weightOffset + inputIdx]);
    ...
    template<
        ...
        u32 Inputs,
        u32 Outputs,
        typename Activation::OutputType Q,
        output::OutputBucketing OutputBucketing>
        ...
        inline auto forward(
            ...
            for (u32 outputIdx = 0; outputIdx < Base::OutputCount; ++outputIdx) {
                ...
                for (u32 inputIdx = 0; inputIdx < PairCount; inputIdx += ChunkSize) {
                    ...
                    const auto input2Vec =
                        load<typename Base::InputType>(&stmInputs[inputIdx + PairCount]);
                ...
                for (u32 inputIdx = 0; inputIdx < PairCount; inputIdx += ChunkSize) {
                    ...
                    const auto input2Vec =
                        load<typename Base::InputType>(&nstmInputs[inputIdx + PairCount]);
    ...
    template<
        ...
        typename Activation::OutputType Q,
        output::OutputBucketing OutputBucketing = output::Single>
    using DensePerspectiveAffine = std::conditional_t<
        PairwiseMul,
        DensePerspectivePairwiseMulAffine<
            Input,
            Param,
            Activation,
            Inputs,
