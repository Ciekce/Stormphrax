...
namespace stormphrax::eval::nnue {
    ...
    private:
        ...
        static inline auto subSubAdd(
            ...
            for (u32 i = 0; i < OutputCount; ++i) {
                dst[i] =
                    src[i] + delta[addOffset + i] - delta[subOffset0 + i] - delta[subOffset1 + i];
            }
        }
        ...
        static inline auto subSubAddAdd(
            ...
            for (u32 i = 0; i < OutputCount; ++i) {
                dst[i] = src[i] + delta[addOffset0 + i] - delta[subOffset0 + i]
                       + delta[addOffset1 + i] - delta[subOffset1 + i];
            }
        }

        static inline auto
        add(std::span<Type, OutputCount> accumulator,
            std::span<const Type, WeightCount> delta,
            u32 offset) -> void {
        ...
        static inline auto
        sub(std::span<Type, OutputCount> accumulator,
            std::span<const Type, WeightCount> delta,
            u32 offset) -> void {
            assert(offset + OutputCount <= delta.size());
    ...
    struct FeatureTransformer {
        ...
        using RefreshTable = RefreshTable<
            FeatureTransformer<Type, Outputs, FeatureSet>,
